var roleSpawn = {
    run(spawn) {
       ...
       |                                         Спаун: меньше 550 энергии                                          |
        \**********************************************************************************************************/
        if (amountEnergy <= 549) {
            if (Memory.room.E51S39.amountMiners0IsLive < Memory.amountCreeps.amountMiners0InE51S39) {
                var newName = "miner0 | " + numberCreep;
                originSpawn.spawnCreep([WORK, WORK, MOVE, MOVE], newName,
                    { memory: { role: "miner0", sourceId: sources[0].id } });
            } else if (Memory.room.E51S39.amountTransportersIsLive < Memory.amountCreeps.amountTransportersInE51S39) {
                var newName = "transporter | " + numberCreep;
                originSpawn.spawnCreep([WORK, CARRY, MOVE, MOVE], newName,
                    { memory: { role: "transporter" } });
            } else if (Memory.room.E51S39.amountMiners1IsLive < Memory.amountCreeps.amountMiners1InE51S39) {
                var newName = "miner1 | " + numberCreep;
                originSpawn.spawnCreep([WORK, WORK, MOVE, MOVE], newName,
                    { memory: { role: "miner1", sourceId: sources[1].id } });
            } else if (Memory.room.E51S39.amountBuildersIsLive < Memory.amountCreeps.amountBuildersInE51S39) {
           ...
           |                                   Спаун: меньше 799 энергии и больше 549                                   |
            ...
            if (Memory.room.E51S39.amountMiners0IsLive < Memory.amountCreeps.amountMiners0InE51S39) {
                ...
                originSpawn.spawnCreep([WORK, WORK, WORK, CARRY, MOVE, CARRY, MOVE], newName,
                    { memory: { role: "builder" } });
            } else if (Memory.room.E51S39.amountWarriorsIsLive < Memory.amountCreeps.amountWarriorsInE51S39) {
                var newName = "warrior | " + numberCreep;
                originSpawn.spawnCreep([RANGED_ATTACK, RANGED_ATTACK, MOVE, MOVE, RANGED_ATTACK, MOVE], newName,
                    { memory: { role: "warrior" } });
            } else if (Memory.room.E51S39.amountClaimersIsLive < Memory.amountCreeps.amountClaimersInE51S39) {
                var newName = "claimer | " + numberCreep;
                originSpawn.spawnCreep([CLAIM, MOVE, MOVE], newName,
                    { memory: { role: "claimer" } });
            } else if (Memory.room.E51S39.amountFarBuildersIsLive < Memory.amountCreeps.amountFarBuildersInE51S39) {
