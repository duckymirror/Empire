                ...
                if (creep.memory.work == "getResource") {
                    ...
                    if ((creep.room.terminal && creep.room.terminal.store[RESOURCE_UTRIUM] > 0) && (creep.room.storage && creep.room.storage.store[RESOURCE_UTRIUM] < 20000)) {
                        if(creep.withdraw(creep.room.terminal, RESOURCE_UTRIUM) == ERR_NOT_IN_RANGE) {
                            creep.moveTo(creep.room.terminal, {ignoreRoads: true, heuristicWeight: 1.2, range: 1, reusePath: 50});
                        }
                    } else if (droppedEnergy) {
                        if (creep.pickup(droppedEnergy) == ERR_NOT_IN_RANGE) {
                            creep.moveTo(droppedEnergy, {ignoreRoads: true, heuristicWeight: 1.2, range: 1});
                        }
                    } else if (ruin) {
                        ...
                        if(creep.withdraw(container, RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
                            creep.moveTo(container, {ignoreRoads: true, heuristicWeight: 1.2, range: 1, reusePath: 50});
                        }
                    ...
                    if (creep.store[RESOURCE_ENERGY] > 0) {
                        ...
                        if (spawnEnergy) {
                            if (creep.transfer(spawnEnergy, RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
                                creep.moveTo(spawnEnergy, {ignoreRoads: true, heuristicWeight: 1.2, range: 1, reusePath: 50});
                            }
                        } else if (creep.room.controller.ticksToDowngrade < 5000) {
                            if (creep.upgradeController(creep.room.controller) == ERR_NOT_IN_RANGE) {
                                creep.moveTo(creep.room.controller, {ignoreRoads: true, heuristicWeight: 1.2, range: 3, reusePath: 50});
                            }
                        } else if (constructionSiteWallAndRampart.length > 0) { 
                            if (creep.build(constructionSiteWallAndRampart[0]) == ERR_NOT_IN_RANGE) {
                                creep.moveTo(constructionSiteWallAndRampart[0], {ignoreRoads: true, heuristicWeight: 1.2, range: 3, reusePath: 50});
                            }
                        }else if (tower.length > 0) {
                            tower.sort((a,b) => a.store[RESOURCE_ENERGY] - b.store[RESOURCE_ENERGY]);
                            if (creep.transfer(tower[0], RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
                                creep.moveTo(tower[0], {ignoreRoads: true, heuristicWeight: 1.2, range: 1, reusePath: 50});
                            }
                        } else if (constructionSite.length > 0) {
                            if (creep.build(constructionSite[0]) == ERR_NOT_IN_RANGE) {
                                creep.moveTo(constructionSite[0], {ignoreRoads: true, heuristicWeight: 1.2, range: 3, reusePath: 50});
                            }
                        } else if (creep.room.storage && creep.room.storage.store[RESOURCE_ENERGY] < 50000) {
